...
export class GooglePlayValidationService {
  ...
  async inspect(input: GooglePurchaseInput): Promise<GooglePurchaseStatus> {
    ...
    if (response.status === 404 || response.status === 410) {
      return { productId: input.productId, purchaseToken: input.purchaseToken, purchasedAt: new Date(), active: false };
    }
    ...
    const body = await response.json() as {
      purchaseState?: number; productId?: string; purchaseTimeMillis?: string; acknowledgementState?: number; obfuscatedExternalAccountId?: string;
    };
    ...
    return {
      ...
      // `purchases.products.get` identifies the SKU in the request path; the
      // OneTimeProductPurchase response does not reliably echo a productId.
      // Requiring a response.productId here would reject every valid purchase.
      active: body.purchaseState === 0 && accountMatches
    };
  }
  ...
  private async acknowledge(input: Pick<GooglePurchaseInput, "productId" | "purchaseToken">): Promise<void> {
    ...
    let response: Response;
    try {
      response = await fetch(`${this.purchaseUrl(input)}:acknowledge`, {
        method: "POST",
